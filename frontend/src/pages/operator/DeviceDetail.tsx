                ...
                {workspace.queue && workspace.queue.length > 0 ? (
                  <div className="space-y-3.5">
                    {workspace.queue.map((cmd: any) => {
                      const isPending = cmd.status === 'pending' || cmd.status === 'queued';
                      const isSending = cmd.status === 'sending' || cmd.status === 'dispatching';
                      const isApplied = cmd.status === 'applied';
                      const isAppliedPending = cmd.status === 'applied_pending_verification';
                      const isVerifying = cmd.status === 'verifying';
                      const isVerified = cmd.status === 'verified';
                      const isVerificationFailed = cmd.status === 'verification_failed';
                      ...
                      if (isPending) {
                        ...
                        badgeLabel = 'QUEUED / PENDING';
                        badgeHelp = 'Waiting for a CPE CWMP session.';
                      } else if (isSending) {
                        badgeClass = 'bg-blue-50 text-blue-800 border-blue-300 animate-pulse';
                        badgeLabel = 'SENDING / DISPATCHED';
                        badgeHelp = 'Configuration request is being sent or is awaiting a CPE response.';
                      } else if (isAppliedPending) {
                        badgeClass = 'bg-purple-50 text-purple-800 border-purple-300 animate-pulse';
                        badgeLabel = 'APPLIED_PENDING_VERIFICATION';
                        badgeHelp = 'CPE may have applied the change, but verification is pending.';
                      } else if (isVerifying) {
                        badgeClass = 'bg-indigo-50 text-indigo-800 border-indigo-300 animate-pulse';
                        badgeLabel = 'VERIFYING';
                        badgeHelp = 'Querying CPE parameters via GetParameterValues.';
                      } else if (isVerified) {
                        badgeClass = 'bg-emerald-50 text-emerald-800 border-emerald-300';
                        badgeLabel = 'VERIFIED';
                        badgeHelp = 'Follow-up GetParameterValues confirmed the expected values.';
                      } else if (isVerificationFailed) {
                        badgeClass = 'bg-orange-50 text-orange-800 border-orange-300';
                        badgeLabel = 'VERIFICATION_FAILED';
                        badgeHelp = 'CPE reconnected but the expected values were not confirmed.';
                      } else if (isApplied || isSuccess) {
                        badgeClass = 'bg-teal-50 text-teal-800 border-teal-300';
                        badgeLabel = 'APPLIED / SUCCESS';
                        badgeHelp = 'CPE returned a successful SetParameterValuesResponse.';
                      } else if (isFailed) {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex flex-col space-y-1">
                              <div className="flex items-center space-x-2.5">
                                <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold border ${badgeClass} flex items-center space-x-1.5`}>
                                  ...
                                  {isAppliedPending && <RefreshCw className="w-3 h-3 text-purple-600 animate-spin" />}
                                  {isVerifying && <RefreshCw className="w-3 h-3 text-indigo-600 animate-spin" />}
                                  {isVerified && <CheckCircle2 className="w-3 h-3 text-emerald-600" />}
                                  {isVerificationFailed && <AlertTriangle className="w-3 h-3 text-orange-600" />}
                                  {(isApplied || isSuccess) && <CheckCircle2 className="w-3 h-3 text-teal-600" />}
                                ...
                                {cmd.retryCount !== undefined && cmd.retryCount > 0 && (
                          ...
                          {cmd.verificationResult && !cmd.verificationResult.verified && cmd.verificationResult.mismatches && cmd.verificationResult.mismatches.length > 0 && (
                            <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg text-xs text-orange-900 font-medium space-y-1">
                              <div className="flex items-center space-x-1.5 font-bold text-orange-800">
                                <AlertTriangle className="w-3.5 h-3.5 text-orange-600" />
                                <span>Verification Parameter Mismatches:</span>
                              </div>
                              <ul className="list-disc list-inside space-y-0.5 font-mono text-[11px]">
                                {cmd.verificationResult.mismatches.map((m: string, idx: number) => (
                                  <li key={idx}>{m}</li>
                                ))}
                              </ul>
                            </div>
                          ...
                          {/* Confirmed Verified Parameters Callout */}
                          {cmd.verificationResult && cmd.verificationResult.verified && (
                            <div className="p-2.5 bg-emerald-50 border border-emerald-200 rounded-lg text-xs text-emerald-900 font-medium flex items-center space-x-2">
                              <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                              <span>CPE parameters read back and verified successfully via GetParameterValues.</span>
                            </div>
                          )}
